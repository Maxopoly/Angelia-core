...
/**
 * If an item transaction failed, the client has to apologize with this packet, acknowledging that the transaction
 * failed, otherwise further transactions will all be blocked
 *
 */
public class ApologizeTransactionPacket extends WriteOnlyPacket {
	...
	public ApologizeTransactionPacket(byte windowID, short actionID) throws IOException {
		...
		writeByte(windowID);
		writeShort(actionID);
		// this packet is only sent if a transaction is failed, so we might as well always write false here
		writeBoolean(false);
	}
