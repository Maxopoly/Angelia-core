...
public class ChunkDataPacketHandler extends AbstractIncomingPacketHandler {
    ...
    private Chunk readChunk(int chunkX, int chunkZ, boolean groundUpContinuos, int bitMask, BinaryReadOnlyData data)
        ...
        // This methods logic was mostly taken from the protocol wiki, thanks to whoever wrote it there
                ...
                for (int z = 0; z < Chunk.CHUNK_WIDTH; z++) {
                    for (int x = 0; x < Chunk.CHUNK_WIDTH; x++) {
                        ...
                        if (startLong == endLong) {
                            ...
                            blockData = (int) (((dataArray[startLong] >> startOffset) & (1 << endOffset) - 1) | (dataArray[endLong] << endOffset));
                        }
                        ...
                        if (usePalette) {
                            if (blockData < palette.length) {
                                ...
                                connection.getLogger().info(
                                      "Data outside of palette; ID: " + blockData + " Length: " + palette.length + " index: " + blockNumber + "  " + read);
                            }
                        }
