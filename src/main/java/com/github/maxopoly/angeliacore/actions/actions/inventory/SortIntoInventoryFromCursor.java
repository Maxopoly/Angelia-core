...
/**
 * Sorts a given item into the given inventory by filling up empty and not
 * completly filled stacks of the same type. Always use Inventory.hasSpaceFor()
 * before using this
 *
 */
	...
	public void execute() {
		...
		for (int i = 0; i < storage.getSize(); i++) {
			...
			if (invSlot == null || invSlot.isEmpty()) {
				invClick = new ClickInventory(connection, windowID, inv.translateStorageSlotToTotal(i), (byte) 0, 0,
						invSlot);
